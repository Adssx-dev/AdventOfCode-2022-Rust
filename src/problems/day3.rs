...
pub fn day3_pt1() -> i32 {
    ...
    reader.lines()
        ...
        .map(|(s1, s2)| s1.chars().into_iter() // Iterate over all chars from first half
            .find(|c_in_str1| s2.clone().chars().any(|c_in_str2| *c_in_str1 == c_in_str2))) // Keep only characters present in both strings
...
pub fn day3_pt2() -> i32 {
    ...
    for i in 0..tmp_vec.len() / 3{
        ...
        let value = tmp_vec[idx].chars().into_iter()
            ...
            .find(|c_in_str1| tmp_vec[idx + 2].chars().into_iter().any(|c_in_str3| *c_in_str1 == c_in_str3)) // Keep only chars present in string 3
